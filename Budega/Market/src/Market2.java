...
class Market{
    ...
    public boolean call(int index){//index é o caixa para qual a pessoa vai
        if(this.queue.isEmpty()){ //Fila não possui clientes, ArrayList vazio, usamos o código .isEmpty
            IO.println("fail: sem clientes");
            return false;
        }
        if(this.counters.get(index)!=null){ //Se tiver alguém no caixa
            IO.println("fail: caixa ocupado");
            return false;
        }
        this.counters.set(index, queue.remove(0)); //a pessoa que está na posição 0 é removida, e setada pro index do caixa. this.counters.set(onde vai a informação, de que posição a informação vem);
        return true;
    }
